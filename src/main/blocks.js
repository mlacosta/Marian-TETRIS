...
export class Block {
  constructor(params, position, color) {
    ...
    this.body = [
      ...
      {
        x: this.position.x + this.gameUnit,
        y: this.position.y + this.gameUnit,
      },
    ];
  ...
  setBodyAbsolute() {
    ...
    for (let i = 0; i < this.body.length; i++) {
      this.bodyAbsolute.push({
        x: this.body[i].x - Xmin,
        y: this.body[i].y - Ymin,
      });
    }
  }
  ...
  drawUnit(unit, context) {
    ...
    context.fillRect(unit.x, unit.y, this.gameUnit, this.gameUnit);
  }
  ...
  collisionDetection = (game) => {
    ...
    for (let i = 0; i < 4; i++) {
      if (this.bodyCoor[i].y <= 1 && this.bodyCoor[i].y >= 0) {
        if (
          game.gameMatrix[this.bodyCoor[i].x][this.bodyCoor[i].y] !==
          game.bgColor
        ) {
...
export class Lstick extends Block {
  constructor(params, position, color) {
    ...
    this.body = [
      ...
      {
        x: this.position.x + 2 * this.gameUnit,
        y: this.position.y + this.gameUnit,
      },
    ];
